...
This contrasts with GRPO's reported success in the LLM domain (DeepSeekMath). A possible explanation is that fine-tuning LLMs might primarily involve refining existing capabilities or making adjustments within a relatively stable policy landscape based on preference signals (which GRPO's group-based advantage estimation might approximate well). This could be different from the RL exploration needed in LunarLander to discover qualitatively different, more efficient strategies (e.g., quick landing and engine shutdown). While these experiments provide an interesting data point, this observation regarding GRPO's exploration/optimization capabilities outside the LLM domain should be considered preliminary and requires further investigation rather than being taken as a definitive limitation.
